...
- Short hypothesis: In selected beta-thalassemia patients, controlled iron
  restriction through hepcidin-ferroportin-`TMPRSS6` biology may reduce iron
...
- Biological target: hepcidin, ferroportin, `TMPRSS6`, `ERFE`, transferrin
  saturation, and iron loading.
- Expected effect: lower excess iron traffic, lower transferrin saturation, less
  erythroid iron stress, and better iron-burden trajectory.
- Patient subgroup: strongest first fit is `NTDT` with iron overload; `TDT`
  relevance is adjunct/comparator only because transfusional iron remains a
  major driver.
...
- Known risks: anemia worsening from excessive iron restriction, infection or
  inflammatory-state confounding, liver and kidney burden, and unknown long-term
  risk for newer investigational agents.
...
- Endpoint: ferritin trend, transferrin saturation, serum iron, `NTBI` or labile
  plasma iron if available, liver iron concentration by MRI, cardiac `T2*`,
  hepcidin, `ERFE`, reticulocytes, hemolysis markers, hemoglobin, and
  transfusion burden.
...
This card argues against iron-supplement shortcuts. Quran 57:25 supports
careful reflection on iron's strength and benefits, while thalassemia biology
requires balance and measurement because iron overload can be dangerous.
