# Hypothesis Cards

Hypotheses are not claims. They are structured candidates for review.

## Promotion Gate

A hypothesis can move forward only if it has:

- disease mechanism;
- patient group;
- evidence in humans or strong preclinical rationale;
- known risks and contraindications;
- interaction review with transfusion, chelation, liver disease, and immune
  disease;
- measurable endpoint;
- affordability path;
- falsification criteria;
- clinician review status.

## Current Priority Order

1. Confirm and optimize existing care baseline.
2. Evaluate approved disease-modifying options and access barriers.
3. Map genotype-specific mechanisms.
4. Compare low-cost adjunct ideas only after safety screening.
5. Explore Quran/bee-product hypotheses only as testable biology, not as cure
   claims.

## First Endpoints

- Reduced transfusion burden.
- Stable or improved pre-transfusion hemoglobin.
- Better ferritin, liver iron, or cardiac T2* trend.
- Fewer transfusion reactions or compatibility problems.
- Better fatigue/function, when measured by validated tools.
